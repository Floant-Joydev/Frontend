...
const Product = () => {
    ...
    let product;
    if( allProduct ){
        const productInd = allProduct.findIndex((obj) => obj._id === param.product_id );
    ...
    let similerProduct;
    if(allProduct){
        similerProduct = allProduct.filter((obj) => obj.Category === product.Category );
    }
  ...
  return (
    ...
    { product && <> <section className="product-heading p1">
        <h3>Home  <strong>{">"}</strong><span>{product.SubCategory}</span></h3>
    </section>
    ...
    <section className="product-body p1">
        ...
        <div className="right">
            ...
            <div className="price">
                ...
                <p><span>Rs. {product.Price}</span> Rs. {product.SalePrice}</p>
            </div>
            <p className='price-contenet' style={{fontWeight: "100"}}>(MRP Inclusive of all taxes)</p>
            <div className="size">
                <p className='select-heading'>Select Plant Size</p>
                <div>
                    <button className={(size === 'Small')?"active": ""} onClick={() => setSize("Small")}>Small</button>
                    <button className={(size === 'Medium')?"active": ""} onClick={() => setSize("Medium")}>Medium</button>
                    <button className={(size === 'Large')?"active": ""} onClick={() => setSize("Large")}>Large</button>
                </div>
            </div>
            ...
            <div className="btn">
                <button onClick={() => {
                    ...
                    if( user ){
                        ...
                        dispatch( createCartAsync({productId: product._id, quantity: quantity, size: size, token: token}) )
                    }else{
                        Navigate('/login/0');
                    }
